...
def ConvertToSymbols(g : GLOBALS, input : str) -> str:
    ...
    #Olchiki vowels list ----Modifications for olchiki added--START
    ol_vowels = ['a','aa','i','ii','u','uu','ee','oo']
    modifier_symbol = ['ᱸ', 'ᱻ', ':']
    ol_vowels_v2 = ['ᱚ','ᱟ','ᱤ','ᱩ','ᱮ','ᱳ']
    mapping_gjdb = {'gxx': 'g', 'jxx': 'j', 'dxx': 'd', 'bxx': 'b'}
    mapping_rella = {'ᱤ': 'i', 'ᱮ': 'e', 'ᱩ': 'u'}
    mapping_nukta = {'ᱚ': 'a', 'ᱮ': 'ei', 'ᱟ': 'a', 'ᱳ': 'a'}
    g.words.syllabifiedWord = "&"
    ...
    for j in range(len(str1)):
        ...
        if (ord(str1[j]) < 8204):
            if(g.langId == 10):
            	if str1[j] in modifier_symbol:
            		print("inside modifiers loop")
            		#if str1[j] == 'ᱸ':
            			#g.words.syllabifiedWord += "&" + "mq"
            		if str1[j] == 'ᱻ':
            			g.words.syllabifiedWord += mapping_rella[str1[j-1]]
            		elif str1[j] == ':':
            			g.words.syllabifiedWord += "gxx"
            	elif (j + 1 < len(str1)) and str1[j+1] == 'ᱹ':
            		print("inside nukta loop")
            		g.words.syllabifiedWord += "&" + mapping_nukta[str1[j]]
            		continue
            	...
            	for i in range(0,128):
            	    ...
            	    if(g.symbolTable[i][0]== str1[j]):
                        ...
                        #print("str_j=", str1[j], "str_j+1=", str1[j+1])
                        ...
                        elif g.symbolTable[i][1] in mapping_gjdb:
                        	print("Inside gjdb loop")
                        	new_value = g.symbolTable[i][1]
                        	if (j + 1 < len(str1) and (str1[j+1] in ol_vowels_v2 or str1[j+1] == 'ᱽ' or str1[j+1] == 'ᱷ')):
                        		print("Inside vowel loop")
                        		g.symbolTable[i][1] = mapping_gjdb[g.symbolTable[i][1]]
                        		g.words.syllabifiedWord += "&" + g.symbolTable[i][1]
                        		print("g.symbolTable[i][1] == ", g.symbolTable[i][1])
                        		g.symbolTable[i][1] = new_value
    ...
    g.words.syllabifiedWord = g.words.syllabifiedWord[1:] #----Modifications for olchiki added--END
    print((g.words.syllabifiedWord),'******')   
    #----Added on 13-11-2024 to handle 'nk' in hindi-santali words. 
    nk_prefix = {
    "aa&nk": "axv",
    "aav&nk": "axv",
    "oo&nk": "axv",
    ...
    for old, new in nk_prefix.items():
        g.words.syllabifiedWord = g.words.syllabifiedWord.replace(old, new)   ### 174-182 Lines added on 13-11-2024
...
def WritetoFiles(g : GLOBALS) -> int:
    ...
    #print(g.syllableList,type(g.syllableList),'**********')
    validSyllable = 0
    for i in range(0,g.syllableCount):
        if g.syllableList[i] != '':
            validSyllable += 1
    ...
    # //syllable
        ...
        syllablesPrint = 0
        rep_dict = {'&g&': '&gxx&', '&j&': '&jxx&', '&d&': '&dxx&', '&b&': '&bxx&', '&eev&': '&ee&', '&oov&': '&oo&','&uvu&': '&uv&', '&ivi&': '&iv&'}
        print("Syllable_count:", g.syllableCount)
        for i in range(g.syllableCount):
            ...
            print(f'##########syllable list {g.syllableList[i]}')
            for old, new in rep_dict.items():
            	g.syllableList[i] = g.syllableList[i].replace(old, new)
            #print(f'##########syllable list {g.syllableList[i]}')
            if g.flags.DEBUG:
                print(f'*******syllable list {g.syllableList[i]}')
            g.words.outputText += '(( "'
